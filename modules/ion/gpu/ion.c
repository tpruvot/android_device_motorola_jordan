...
/* Unexported, in kernel/fork.c */
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, ___put_task_struct, struct task_struct *tsk);
//SYMSEARCH_DECLARE_FUNCTION_STATIC(void, _exit_creds, struct task_struct *tsk);
...
/* Override in module source files */
#define put_task_struct(tsk) ion_put_task_struct(tsk)
				      ...
				      unsigned long arg))
{
	...
	if (!idev) {
		pr_err("ion: ion_device_create kzalloc failed.\n");
		return ERR_PTR(-ENOMEM);
	}
	...
	devid = MKDEV(MISC_MAJOR, idev->dev.minor);
	pr_info("ion: registered misc device %x (%d:%d)\n", devid, MISC_MAJOR, idev->dev.minor);
