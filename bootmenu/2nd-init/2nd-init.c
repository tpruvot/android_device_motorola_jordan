...
int main(int argc, char** argv)
{
  ...
  //wait for interrupt
  ppeek = (long) waitpid(1, &status, WSTOPPED);
  //ppeek = (long) wait(&status);

  //double check we are attached
  if (!WIFSTOPPED(status)) {
    printf(" warning wait=%ld status=%d, but continue\n", ppeek, status);
  }

  timeout = 30;
  ppeek = ptrace(PTRACE_GETREGS, 1, NULL, &regs);
  while (ppeek == -1 && errno == ESRCH && timeout--) {
    //on tegra2 (dual core), a second try is sometimes required...
    printf(" err %d retrying...\n", errno);
    waitpid(1, &status, WSTOPPED);
    ppeek = ptrace(PTRACE_GETREGS, 1, NULL, &regs);
  }

  //check if PC is valid
  if (regs.ARM_pc == 0 || ppeek == -1)
  {
    printf("ERROR: Couldn't get PC register value. err %d\n", errno);
    return 1;
  }

  printf("/init PC is at 0x%08lx. %s\n", regs.ARM_pc,
         (regs.ARM_cpsr & PSR_T_BIT)?"Thumb mode":"ARM mode");
  ...
  //regs.ARM_ip = injected_code_address;
  printf("Setting /init PC to: 0x%08lX.\n", injected_code_address);
  ppeek = ptrace(PTRACE_SETREGS, 1, NULL, &regs);
  if (ppeek == -1) {
    printf("ptrace failed (%d), retry a second time...\n", errno);
    ppeek = ptrace(PTRACE_SETREGS, 1, NULL, &regs);
  }

  memset(&regs, 0, sizeof(regs));
  ppeek = ptrace(PTRACE_GETREGS, 1, NULL, &regs);
  if (ppeek == -1 && errno == ESRCH) {
    // on tegra2 (dual core), a second try is required...
    printf(" err %d retrying...\n", errno);
    ppeek = ptrace(PTRACE_GETREGS, 1, NULL, &regs);
  }
  ...
  if (regs.ARM_pc == 0 || ppeek == -1)
  {
    printf("ERROR: Couldn't check PC register value. err %d\n", errno);
    ptrace(PTRACE_CONT, 1, NULL, NULL);
    return 1;
  }

  if (regs.ARM_pc != injected_code_address) {
      printf(" ARM_pc=0x%x\n", (uint32_t) regs.ARM_pc);
  }

  //fire it
  printf("Detaching...\n");
  ppeek = ptrace(PTRACE_DETACH, 1, NULL, NULL);
  if (ppeek == -1) {
    printf("ptrace failed, err %d\n", errno);
    usleep(10000);
    ptrace(PTRACE_DETACH, 1, NULL, NULL);
  }
  return 0;
